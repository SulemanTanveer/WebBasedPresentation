...
(function ($, window, undefined) {
	...
	var wptSlider = function (element, opts) {
		var
			settings = $.extend({}, $.fn.wptslider.defaults, opts),
			$slider = $(element),
			effects = $.fn.wptslider.effects,
			preparations = $.fn.wptslider.preparations,
			//internal variables
			vars = {
				active: '',
				next: '',
		...
		var
			setUpSettings = function (settings) {
				...
				settings.cycled = String(settings.cycled) ==  false;
...
var value= function(){
	...
	return val;
		...
		this.uninit = function () {
			...
			$(element).data('wptslider', null);
		};

		init($slider, settings, vars);
	};

	$.fn.wptslider = function (opts) {
		return this.each(function () {
			var element = $(this);
			if (element.data('wptslider')) {
				return element.data('wptslider');
			}
			
			element.data('slider:original', element.clone());
			var wptslider = new wptSlider(this, opts);
			element.data('wptslider', wptslider);
        });
	};

	$.fn.wptslider.defaults = {
